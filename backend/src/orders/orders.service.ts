...
export const ordersService = {
  ...
  getByRef: async (ref: string): Promise<any | null> => {
    const order = await db.query.orders.findFirst({
  ...
  generateOrderRef: async (): Promise<string> => {
  ...
  updateStatus: async (id: string, status: string): Promise<any | null> => {
    const orderId = parseInt(id, 10);
    if (isNaN(orderId)) {
      throw new Error("Invalid order ID");
    }
  ...
  updatePaymentStatus: async (id: string, paymentStatus: string): Promise<any | null> => {
    const orderId = parseInt(id, 10);
    if (isNaN(orderId)) {
      throw new Error("Invalid order ID");
    }
  ...
  cancel: async (id: string): Promise<any | null> => {
    const orderId = parseInt(id, 10);
    if (isNaN(orderId)) {
      throw new Error("Invalid order ID");
    }
